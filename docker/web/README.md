...
Isso mantém os três Dockerfiles coerentes entre si: API e Worker também recebem `BUILD_CONFIGURATION` como argumento, ainda que ali o valor seja a configuração de compilação do .NET (`Release`).
...
>
> Note que essa questão **não existe no backend**: API e Worker leem configuração em runtime a partir de variáveis de ambiente injetadas pelo Container Apps, com segredos vindo do Key Vault via Managed Identity. A restrição é específica de aplicações front-end compiladas.
...
Apesar da aparência, **não é um comentário**: é uma *parser directive* lida pelo BuildKit antes do parsing do arquivo, e precisa ser a primeira linha. Ela fixa a versão da sintaxe do Dockerfile, garantindo comportamento consistente entre diferentes versões do Docker Engine.
...
O build context é a **raiz do repositório `web`**. Note que `nginx.conf` precisa estar acessível a partir dessa raiz — no repositório real ele ficaria ao lado do `Dockerfile`, e não em `docker/web/` como neste repositório de entrega.
...
O `Cache-Control: no-store` no `index.html` é igualmente importante: como o arquivo referencia os bundles por nome com hash, um `index.html` cacheado apontaria para bundles de um deploy anterior que já não existem mais, resultando em uma aplicação quebrada após cada deploy.
...
Essa combinação é o que permite deploys sem invalidação manual de cache: o `index.html` é sempre buscado do servidor e aponta para os bundles corretos, enquanto os bundles — imutáveis por definição — permanecem no cache do navegador entre visitas.

A diretiva `immutable` reforça esse comportamento: sem ela, o navegador ainda enviaria requisições de revalidação (`If-None-Match`) ao recarregar a página, mesmo com o recurso em cache e dentro da validade. Com `immutable`, ele assume que o arquivo nunca muda e nem consulta o servidor.

O `max-age` é declarado dentro do próprio `add_header`, em vez de usar a diretiva `expires`. As duas fazem a mesma coisa — `expires 1y` também emite `Cache-Control: max-age=31536000` —, e usá-las em conjunto produziria um cabeçalho `Cache-Control` duplicado na resposta.
...
Este endpoint existe por causa de uma decisão tomada nos outros Dockerfiles: **as imagens são distroless e não possuem shell**, o que inviabiliza um `HEALTHCHECK` interno baseado em `curl` ou `wget`.

A verificação passa a ser responsabilidade da plataforma. No Container Apps, este caminho é o alvo dos probes:
...
| *Readiness* | Remove temporariamente do balanceamento uma réplica que não está apta a responder |
...
`server_tokens off` permanece no nível `server` porque não é um `add_header` — é uma diretiva própria e é herdada normalmente. Ela remove a versão do nginx do header `Server` e das páginas de erro; é *security through obscurity*, de valor limitado isoladamente, mas evita entregar de graça a informação de qual versão exata está em execução para quem faz varredura automatizada.

> **O que não está aqui:** `Content-Security-Policy` e `Strict-Transport-Security`. A CSP depende do que a aplicação real carrega (CDNs, fontes externas, scripts inline do Angular) e uma política genérica quebraria a aplicação ou seria permissiva demais para ter valor. O HSTS é desnecessário neste ponto porque o TLS termina no ingress do Container Apps, que já o aplica — configurá-lo aqui seria redundante e potencialmente conflitante.
...
Bundles JavaScript de aplicações Angular são grandes e comprimem muito bem — reduções de 60% a 80% são típicas. `gzip_min_length 1024` evita comprimir arquivos pequenos, onde o custo de CPU supera a economia de banda.
...
Note que a lista de tipos **não inclui imagens rasterizadas** (`png`, `jpg`, `gif`): esses formatos já são comprimidos, e reprocessá-los apenas consome CPU. O `image/svg+xml` está na lista porque SVG é XML — texto, e comprime bem.
